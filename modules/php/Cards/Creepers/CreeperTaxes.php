...
class CreeperTaxes extends CreeperCard
{
    ...
    private function checkResolveTaxesAndMoney($onlyForActivePlayer)
    {
        ...
        $game = Utils::getGame();
        if ($onlyForActivePlayer && 
            $game->getActivePlayerId() != $taxesAndMoney["player_id"]) {
            return null;
        }

        $game->setGameStateValue("creeperToResolvePlayerId", $taxesAndMoney["player_id"]);
        $game->setGameStateValue("creeperToResolveCardId", $taxesAndMoney["tax_card"]["id"]);
    ...
    public function resolvedBy($player_id, $args)
    {
        ...
        if ($taxesAndMoney == null || 
            $taxesAndMoney["player_id"] != $player_id) {
            Utils::throwInvalidUserAction(
                fluxx::totranslate("You don't have Taxes and Money in play")
            );
        }
